...
description: 블록체인 보안의 수학적 기반, 타원곡선의 원리와 키 생성
keywords:
  [
    elliptic curve,
    ECC,
...
// 모듈러 연산: 항상 0 ~ p-1 범위로 정리
function mod(n: bigint): bigint {
  return ((n % p) + p) % p;
}

// 모듈러 역원: n * inv ≡ 1 (mod p)
// 작은 p라서 단순 탐색으로 구한다
...
// 테스트
console.log("=== 타원곡선 y² = x³ + 7 (mod 17) ===\n");

const allPoints = findAllPoints();
console.log(`곡선 위의 점 개수 (위수): ${allPoints.length}`);
console.log("점 목록:");
allPoints.forEach((pt, i) => {
  if (pt === null) {
    console.log(`  [${i}] O (무한원점)`);
  } else {
...
console.log(`\n생성점 G = (${G.x}, ${G.y})`);
console.log(`곡선 위에 있는지: ${isOnCurve(G)}`);

console.log("\n=== 스칼라 곱셈 테스트 (k × G) ===");
for (let k = 1n; k <= 10n; k++) {
  const result = scalarMultiply(k, G);
  if (result === null) {
    console.log(`${k}G = O (무한원점)`);
  } else {
...
console.log("=== 이더리움 키 생성 ===\n");

// 고정 개인키 사용 (예시가 매번 동일하게 나오도록)
const privateKey =
...
console.log("개인키 (256비트 정수):");
console.log(`  ${wallet.privateKey}`);
console.log(
  `  길이: ${wallet.privateKey.length - 2} hex chars = ${hexBitLength(
    wallet.privateKey,
  )} bits`,
);

console.log("\n공개키 (비압축, 512비트 = x좌표 + y좌표):");
const publicKey = wallet.signingKey.publicKey;
console.log(`  ${publicKey}`);
const uncompressed = splitUncompressedPubKey(publicKey);
console.log(`  접두사: ${uncompressed.prefix} (0x04 = 비압축)`);
console.log(`  x 좌표: ${uncompressed.x}`);
console.log(`  y 좌표: ${uncompressed.y}`);

// 압축 공개키: x좌표 + y의 짝/홀 정보
const compressed = compressPubKey(publicKey);

console.log("\n공개키 (압축, 264비트):");
console.log(`  ${compressed.compressed}`);
console.log(
  `  접두사: ${compressed.prefix} (${
    compressed.yIsEven ? "y 짝수" : "y 홀수"
  })`,
);

console.log("\n주소 (공개키의 Keccak256 해시 마지막 20바이트):");
...
공개키 (비압축, 512비트 = x좌표 + y좌표):
  ...
  접두사: 0x04 (0x04 = 비압축)
  x 좌표: 4e3b81af9c2234cad09d679ce6035ed1392347ce64ce405f5dcd36228a25de6e
  y 좌표: 47fd35c4215d1edf53e6f83de344615ce719bdb0fd878f6ed76f06dd277956de
