---
title: Hash Function
description: 해시 함수의 원리와 블록체인에서의 활용
...
const modified = "The quick brown fox jumps over the lazy dog."; // 마침표 추가

const hash1 = sha256(original);
const hash2 = sha256(modified);

console.log(`원본: "${original}"`);
console.log(`수정: "${modified}"`);
console.log(`원본 해시: ${hash1}`);
console.log(`수정 해시: ${hash2}`);

const diffBits = countDifferentBits(hash1, hash2);
console.log(
  `다른 비트 수: ${diffBits} / 256 (${((diffBits / 256) * 100).toFixed(1)}%)`,
);
```
...
console.log(
  `\n위조 트랜잭션 검증: ${
    verifyMerkleProof(fakeTx, proof, merkleRoot) ? "성공" : "실패"
  }`,
);
```
...
수정: "The quick brown fox jumps over the lazy dog."
원본 해시: d7a8fbb307d7809469ca9abcb0082e4f8d5651e46d3cdb762d02d0bf37c9e592
수정 해시: ef537f25c895bfa782526529a9b63d97aa631564d5d789c2b765448c8635fb6c
다른 비트 수: 136 / 256 (53.1%)
